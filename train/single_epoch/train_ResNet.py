...
def train_resnet_dataloader_seoul(save_filename,logging_filename,signals_path, train_dataset_list,val_dataset_list,test_dataset_list,batch_size = 10000,
                                          ...
                                          ,class_num=5,classification_mode='5class',gpu_num=0,layers=[2,2,2,2],first_conv=[49,7,24],maxpool=[7,2,3],blocks=[64,128,256,512],kernel_size=9,block=BasicBlock):
    # cpu processor num
    ...
    model = ResNet_200hz(block=block,layers=layers, first_conv=first_conv,maxpool=maxpool, layer_filters=blocks, in_channel=len(use_channel),
                    block_kernel_size=kernel_size,block_stride_size=2, num_classes=class_num, use_batchnorm=True, zero_init_residual=False,
...
def training_resnet_dataloader_seoul(use_channel=[0,1],classification_mode='5class',gpu_num=0):
    ...
    save_filename = model_save_path + f'ResNet18_%.5f_{use_channel}_{first_conv}_{maxpool}_{blocks}_{kernel_size}.pth'%(learning_rate)
    
    logging_filename = logging_save_path + f'ResNet18_%.5f_{use_channel}_{first_conv}_{maxpool}_{blocks}_{kernel_size}.txt'%(learning_rate)
    print('save filename : ',save_filename)

    print(save_filename)
    
    train_resnet_dataloader_seoul(save_filename=save_filename,logging_filename=logging_filename,signals_path=signals_path, train_dataset_list=training_fold_list,val_dataset_list=validation_fold_list,
                                        ...
                                        class_num=class_num,classification_mode=classification_mode,gpu_num=gpu_num,first_conv=first_conv,maxpool=maxpool,
                                        blocks=blocks,kernel_size=kernel_size)

def training_resnet50_dataloader_seoul(use_channel=[0,1],classification_mode='5class',gpu_num=0):
    signals_path = '/home/eslab/dataset/seoulDataset/C3-M2/'
    # signals_path = '/home/eslab/dataset/seoulDataset/1channel_prefilter_butter_minmax_-1_1/signals_dataloader/'
    dataset_list = os.listdir(signals_path)
    dataset_list.sort()
    ...
    save_filename = model_save_path + f'ResNet50_%.5f_{use_channel}_{first_conv}_{maxpool}_{blocks}_{kernel_size}.pth'%(learning_rate)
    
    logging_filename = logging_save_path + f'ResNet50_%.5f_{use_channel}_{first_conv}_{maxpool}_{blocks}_{kernel_size}.txt'%(learning_rate)
    print('save filename : ',save_filename)
    ...
    train_resnet_dataloader_seoul(save_filename=save_filename,logging_filename=logging_filename,signals_path=signals_path, train_dataset_list=training_fold_list,val_dataset_list=validation_fold_list,
                                        ...
                                        scheduler=scheduler,warmup_iter=warmup_iter,cosine_decay_iter=cosine_decay_iter,stop_iter=stop_iter,use_channel=use_channel,
                                        class_num=class_num,classification_mode=classification_mode,gpu_num=gpu_num,first_conv=first_conv,maxpool=maxpool,
                                        blocks=blocks,kernel_size=kernel_size,layers=layers,block=Bottleneck)

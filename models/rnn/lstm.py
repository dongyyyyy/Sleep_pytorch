...
class BiLSTM_sequence_one(nn.Module):
    def __init__(self, input_size=128, class_num=5, hidden_dim=256, num_layers=1, use_gpu=True,sequence_length=10):
        super(BiLSTM_sequence_one, self).__init__()
        self.input_size = input_size
        self.num_directions = 2
        self.num_layers = num_layers
        self.hidden_dim = hidden_dim
        ...
        self.hidden2label = nn.Linear(hidden_dim * 2, class_num)
        self.shortcut = nn.Linear(self.input_size, hidden_dim * 2)
        self.out_epoch = sequence_length // 2
    def init_hidden1(self, batch_size):
        # first = h(hidden) / second = c(cell)
        if self.use_gpu:
            return (
    ...
    def forward(self, x):
        ...
        # print('shortcut shape : ',shortcut_output.shape)
        shortcut_output = self.shortcut(shortcut_output)
        self.hidden1 = self.init_hidden1(x.size(0))
        self.bi_lstm1.flatten_parameters()
        output, self.hidden1 = self.bi_lstm1(x, self.hidden1)
        # print('output shape : ',output.shape)
        ...
        output = output + shortcut_output
        # print('output shape : ',output.shape)
        y = self.hidden2label(output[:,self.out_epoch,:])
        # print('y shape : ',y.shape)
        # 1 / sequence_length / vector
        y = y.squeeze(0)
        return y
...
class lstm_one(nn.Module):
    def __init__(self, flat=3456 * 2, num_layers=1, hidden_dim=512, num_classes=5,sequence_length=10):
        super(lstm_one, self).__init__()
        self.classification = BiLSTM_sequence_one(class_num=num_classes, input_size=flat,
                                                            hidden_dim=hidden_dim, num_layers=num_layers,sequence_length=sequence_length)
        self.dropout = nn.Dropout(p=0.5)
        self.sequence_length = sequence_length
        self.flat = flat
    def forward(self, x):
        # input shape batch / sequence_length * vector
        # batch / sequence_length / vector
        # print('before view : ',x.shape)
        x = x.view(x.size(0)//self.sequence_length,self.sequence_length,x.size(-1))
        # print('view : ',x.shape)
        out = self.dropout(x)
        out = self.classification(out)
...
class lstm_channels(nn.Module):
    def __init__(self, input_size=512, in_channels=3, num_directions=1,hidden_dim=512, num_layers=1, use_gpu=True):
        super(lstm_channels, self).__init__()
        self.input_size = input_size
